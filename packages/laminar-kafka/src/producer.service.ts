...
import { Kafka, Producer, ProducerConfig, RecordMetadata, ProducerRecord } from 'kafkajs';
...
export const producerMiddleware = (producer: ProducerService): Middleware<ProducerContext> => (next) => (payload) =>
...
export const registerSchemas = (register: { [key: string]: ConfluentSchema }): RegisterSchemas => {
...
export class ProducerService implements Service {
  ...
  public async sendWithSchema<TValue>(
    record: EncodedProducerRecord<TValue> & { schema: ConfluentSchema },
